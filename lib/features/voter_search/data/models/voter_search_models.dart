...
class VoterElector {
  ...
  factory VoterElector.fromJson(Map<String, dynamic> json) {
    ...
    final String partNo = (json['uPartNo'] ?? '').toString().trim();
    final String mohallaNo = (json['secNo'] ?? '').toString().trim();
    return VoterElector(
      id: (json['ID'] ?? '').toString(),
      ...
      distNo: (json['DistNo'] ?? json['distNo'] ?? '').toString().trim(),
  ...
  final String serNo;
  final String partNo;
  final String mohallaNo;
  final String urbanWardNo;
  final String ruralWardNo;
  ...
  bool get isUrban {
    ...
    if (type.contains('R')) return false;
  ...
  String get slipBodyName {
    if (isUrban) {
      ...
      if (ubTypeName.isNotEmpty) return ubTypeName;
    }
